...
class TfBridge {
 public:
  ...
  //关键字delete其表示删除默认拷贝构造函数以及默认赋值重载函数
  //也就说禁用了该类的拷贝以及等号赋值功能，强行使用的话会报错
 ...
 private:
  ...
  const double lookup_transform_timeout_sec_; //查找tf时的超时时间
  const tf2_ros::Buffer* const buffer_;//用于存储tf数据
};
