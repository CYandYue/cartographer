...
/** 
 * @brief 构造函数 根据传入的node_options, MapBuilder, 以及tf_buffer 完成三个本地变量的初始化
 * 
...
//通过 map_builder_->AddTrajectoryBuilder() 函数完成了一条轨迹的新建
//轨迹新建会绑定 OnLocalSlamResult() 函数，同时对应一个SensorBridge，
//并且会把该轨迹的参数保存在 MapBuilderBridge::trajectory_options_变量之中。
