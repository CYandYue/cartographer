...
class Node {
 ...
 private:
  ...
  // 简单来说就是使用的时候 必须对 mutex_ 进行加锁
  MapBuilderBridge map_builder_bridge_ GUARDED_BY(mutex_);
  //map_builder_bridge_ 初始化，是在 Node 类构造函数初始化列表中完成的
  //即 node_main.cc里,Node类初始化是传入了一个map_builder的
  //MapBuilderBridge 实现于 src/cartographer_ros/cartographer_ros/cartographer_ros/map_builder_bridge.cc 文件中
